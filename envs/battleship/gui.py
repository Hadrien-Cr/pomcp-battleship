import os
import pygame
from envs.battleship.coord import Coord, Tile
...
class GuiTile(Tile):
    ...
    def draw(self, img=None, color=pygame.Color("white")):
        ...
        pygame.draw.rect(
            self.surface, GuiTile._borderColor, rect, GuiTile._borderWidth
        )  # draw border
...
class GridGui(object):
    ...
    def __init__(self, x_size, y_size, tile_size):
        ...
        for k, v in self._assets.items():
            self.assets[k] = pygame.transform.scale(
                pygame.image.load(v), [tile_size] * 2
            )
    ...
    def build_board(self):
        ...
        for idx in range(self.n_tiles):
            tile = GuiTile(
                self.get_coord(idx), surface=self.surface, tile_size=self.tile_size
            )
            tile.draw(img=None)
